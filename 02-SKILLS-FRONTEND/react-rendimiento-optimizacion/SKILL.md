...
ecc-source: skills/react-performance
...
- Primera palanca SIEMPRE: mover trabajo a Server Components (cero JS al cliente) antes de memoizar en cliente.
- Revalidación quirúrgica: `revalidatePath`/`revalidateTag` tras mutaciones en Server Actions, nunca revalidación global.
- Queries paralelas: `Promise.all` para fetches independientes a Supabase (patrón ya documentado en CLAUDE-MASTER §14).
