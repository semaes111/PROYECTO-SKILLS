...
ecc-source: skills/nextjs-turbopack
...
- Stack base: **Next.js 15/16 App Router + React 19 + TypeScript estricto** (tipo `any` prohibido). Server Components por defecto; `"use client"` solo cuando hay interactividad.
- Deploy dual: **Vercel** (frontend, ej. `whatsapp-clinic-dashboard`) y **Dokploy/VPS** para servicios. Verificar que la config de Turbopack no rompe el build de Docker multi-stage en Dokploy.
- Acceso a datos: patrón `"use server"` + acceso directo a Supabase desde Server Components (sin capa API intermedia salvo necesidad de webhook externo).
