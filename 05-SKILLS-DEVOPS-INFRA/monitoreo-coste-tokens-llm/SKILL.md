...
ecc-source: skills/cost-tracking
...
Esta skill complementa (NO sustituye) la infraestructura de coste ya desplegada:

- `/opt/mimo-cost-monitor.sh` (hourly, alerta Telegram vía `/etc/bot-audit.env`)
- `/opt/bot-audit-collect.sh` (8h → `bot_audit_log` en Supabase) + `bot-audit-analyze.sh` (DeepSeek, ~$0.15/mes)
- Skill local `optimizacion-consumo-tokens-openclaw` (4 vectores de ataque: contexto inerte, estado acumulado, frecuencia, consumo externo)

Regla de oro del ecosistema: **discrepancia entre auditoría interna y facturación externa = actor externo con tus credenciales**, no bug de métricas (`tcpdump VPS→proveedor = 0 paquetes` confirma origen externo). Ante anomalía: rotar key primero, investigar después.
